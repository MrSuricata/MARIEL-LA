...
import { 
  ...
  ZoomIn, Maximize2, Truck, CreditCard, BookOpen, MessageCircle, Copy, Database, XCircle
} from 'lucide-react';
...
const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <StoreContext.Provider value={{
      products, cart, fairs, history, blogPosts, currency, isAdmin,
...
const Navbar = ({ toggleCart }: { toggleCart: () => void }) => {
  ...
  // Determine if we need light text (for dark backgrounds like Home Hero)
  // Logic: If on Home ('/') AND not scrolled -> Transparent background with Light text
  // Else (Scrolled or other pages) -> Default/Light background with Dark text
  const isTransparent = location.pathname === '/' && !scrolled;
  const textColorClass = isTransparent ? 'text-leather-50 hover:text-white' : 'text-leather-900 hover:text-leather-600';
  const logoColorClass = isTransparent ? 'text-leather-50' : 'text-leather-900';
  const iconBgClass = isTransparent ? 'bg-white/20 text-white hover:bg-white/40' : 'bg-white/50 text-leather-900 hover:bg-white';
  ...
  const handleNavClick = (e: React.MouseEvent, target: string, isAnchor: boolean = false) => {
    ...
    if (isAnchor) {
      ...
      if (location.pathname !== '/') {
       ...
       // Added logic: If clicking the link of the page we are already on, scroll to top.
       e.preventDefault();
       window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  };
  ...
  const NavLink = ({ label, to, isAnchor = false }: { label: string, to: string, isAnchor?: boolean }) => {
    ...
    // For active state on transparent navbar, use a brighter accent or white
    const activeColor = isTransparent ? 'text-white' : 'text-leather-600';
    const baseColor = isActive ? activeColor : textColorClass;
    
    return (
      <Link 
        to={to} 
        onClick={(e) => handleNavClick(e, to, isAnchor)}
        className={`font-serif font-medium tracking-wide transition-all duration-300 relative group ${baseColor}`}
      >
        {label}
        <span className={`absolute -bottom-1 left-0 w-0 h-0.5 transition-all duration-300 group-hover:w-full ${isActive ? 'w-full bg-leather-500' : 'bg-leather-400'}`}></span>
      </Link>
    );
  };
    ...
    <nav className={`fixed top-8 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-leather-50/95 backdrop-blur-md shadow-md py-2 top-0' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <Link to="/" onClick={(e) => handleNavClick(e, '/', false)} className={`font-serif text-2xl tracking-wider font-bold z-50 cursor-pointer transition-colors duration-300 ${logoColorClass}`}>
            MARIEL'LA
          </Link>
          ...
          <div className="flex items-center space-x-4 z-50">
            <button onClick={toggleCart} className={`relative p-2 transition-colors rounded-full shadow-sm ${iconBgClass}`}>
              <ShoppingBag size={22} />
              {itemCount > 0 && (
            ...
            <Link to={isAdmin ? "/admin" : "/login"} className={`p-2 rounded-full transition-colors ${iconBgClass}`}>
              <Settings size={20} />
            </Link>
            <button className={`md:hidden p-2 rounded-full ${iconBgClass}`} onClick={() => setIsMenuOpen(!isMenuOpen)}>
              {isMenuOpen ? <X /> : <Menu />}
            </button>
          </div>
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-0 left-0 w-full h-screen bg-leather-50 flex flex-col items-center justify-center space-y-8 z-40 text-leather-900">
            <Link to="/" onClick={(e) => handleNavClick(e, '/', false)} className="font-serif text-2xl font-bold">Inicio</Link>
            <Link to="/catalogo" onClick={(e) => handleNavClick(e, '/catalogo')} className="font-serif text-2xl font-bold">Tienda</Link>
            <Link to="/historia" onClick={(e) => handleNavClick(e, '/historia')} className="font-serif text-2xl font-bold">Historia</Link>
            <Link to="/blog" onClick={(e) => handleNavClick(e, '/blog')} className="font-serif text-2xl font-bold">Descubre</Link>
            <Link to="/ferias" onClick={(e) => handleNavClick(e, '/ferias')} className="font-serif text-2xl font-bold">Ferias</Link>
            <Link to="#contacto" onClick={(e) => handleNavClick(e, '#contacto', true)} className="font-serif text-2xl font-bold">Contacto</Link>
        </div>
      )}
    </nav>
    </>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-1 md:order-2 relative">
             <div className="aspect-[4/5] rounded-lg overflow-hidden shadow-2xl rotate-2 hover:rotate-0 transition-transform duration-700 border-4 border-leather-800 bg-leather-800">
               <img src="https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=800" alt="Taller de cuero" className="w-full h-full object-cover opacity-90 hover:opacity-100 transition-opacity" loading="lazy" />
             </div>
          </div>
        </div>
      </div>
        ...
        <div className="md:w-1/2 p-12 flex flex-col justify-center bg-leather-900 text-white relative overflow-hidden">
          <div className="relative z-10">
            ...
            <div className="space-y-6">
              <a href="https://wa.me/59899123456" className="flex items-center gap-4 group cursor-pointer hover:bg-leather-800 p-2 -ml-2 rounded-lg transition-colors"><Phone size={20} className="text-green-400" /> <span className="font-bold text-lg text-white">+598 99 123 456</span></a>
              <a href="mailto:info@mariella.com.uy" className="flex items-center gap-4 group cursor-pointer hover:bg-leather-800 p-2 -ml-2 rounded-lg transition-colors"><Mail size={20} className="text-blue-400" /> <span className="font-bold text-lg text-white">info@mariella.com.uy</span></a>
              <div className="flex items-center gap-4 group hover:bg-leather-800 p-2 -ml-2 rounded-lg transition-colors"><Instagram size={20} className="text-pink-400" /> <span className="font-bold text-lg text-white">@mariella.uy</span></div>
            </div>
          </div>
          <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/leather.png')]"></div>
        </div>
        <div className="md:w-1/2 bg-cover bg-center min-h-[400px]" style={{backgroundImage: 'url("https://images.unsplash.com/photo-1459411552884-841db9b3cc2a?w=800")'}}></div>
      </div>
    </div>
  </section>
);
...
// Admin Panel Updates
const AdminPanel = () => {
  const { products, fairs, history, blogPosts, logout, addProduct, updateProduct, deleteProduct, addFair, updateFair, deleteFair, addHistoryEvent, updateHistoryEvent, deleteHistoryEvent, addBlogPost, updateBlogPost, deleteBlogPost } = useStore();
  const [activeTab, setActiveTab] = useState<'products' | 'fairs' | 'history' | 'blog' | 'system'>('products');
...
export const CATEGORIES = ['Todas', 'Carteras', 'Billeteras', 'Cintos', 'Mochilas', 'Accesorios'];
  ...
  const ProductForm = () => {
    const [formData, setFormData] = useState<Partial<Product>>(editingProduct || { name: '', description: '', priceUYU: 0, priceUSD: 0, category: 'Carteras', images: [''], materials: [], colors: [], dimensions: '', isFeatured: false });
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-lg border border-leather-200 min-h-[600px] overflow-hidden">
          <div className="border-b border-leather-200 px-8 py-5 flex gap-8 bg-leather-50/50 overflow-x-auto">
            {['products', 'fairs', 'history', 'blog', 'system'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab as any)} className={`text-lg font-bold pb-1 border-b-2 capitalize whitespace-nowrap ${activeTab === tab ? 'border-leather-900 text-leather-900' : 'border-transparent text-leather-400'}`}>{tab === 'products' ? 'Productos' : tab === 'fairs' ? 'Ferias' : tab === 'history' ? 'Historia' : tab === 'blog' ? 'Blog' : 'Sistema'}</button>
            ))}
          </div>
          <div className="p-8">
            {activeTab === 'system' && (
              <div className="animate-fade-in max-w-2xl">
                 ...
                 <p className="text-leather-700 mb-6 leading-relaxed">
                   Esta aplicación funciona localmente. Los cambios que has hecho (nuevos productos, ferias, etc.) están guardados solo en este navegador. 
...
// Switched to Light Theme to fix contrast issues with Navbar on dark background
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen bg-leather-50 relative flex items-center justify-center p-4">
      {/* Texture Background */}
      <div className="absolute inset-0 opacity-40 bg-[url('https://www.transparenttextures.com/patterns/cream-paper.png')] pointer-events-none"></div>
      
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full relative z-10 border border-leather-100">
