...
import { 
  ...
  ZoomIn, Maximize2, Truck, CreditCard, BookOpen, MessageCircle
} from 'lucide-react';
  ...
  const NavLink = ({ label, to, isAnchor = false }: { label: string, to: string, isAnchor?: boolean }) => {
    ...
    return (
      <Link 
        ...
        className={`font-serif font-medium tracking-wide transition-all duration-300 relative group ${isActive ? 'text-leather-600' : 'text-leather-900 hover:text-leather-600'}`}
      >
        {label}
    ...
    <nav className={`fixed top-8 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-leather-50/95 backdrop-blur-md shadow-md py-2 top-0' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <Link to="/" onClick={(e) => handleNavClick(e, '/', false)} className="font-serif text-2xl tracking-wider text-leather-900 font-bold z-50 cursor-pointer">
            MARIEL'LA
          </Link>
          <div className={`hidden md:flex items-center space-x-8 ${!scrolled && location.pathname === '/' ? 'bg-leather-50/80 px-8 py-3 rounded-full backdrop-blur-sm shadow-sm' : ''}`}>
          ...
          <div className="flex items-center space-x-4 z-50">
            <button onClick={toggleCart} className="relative p-2 text-leather-900 hover:text-leather-600 transition-colors bg-white/50 rounded-full hover:bg-white hover:shadow-sm">
              <ShoppingBag size={22} />
              {itemCount > 0 && (
            ...
            <Link to={isAdmin ? "/admin" : "/login"} className="text-leather-800 hover:text-leather-500 bg-white/50 p-2 rounded-full hover:bg-white transition-colors">
              <Settings size={20} />
            </Link>
            <button className="md:hidden p-2 bg-white/50 rounded-full" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              {isMenuOpen ? <X /> : <Menu />}
            </button>
          </div>
        </div>
            ...
            <Reveal key={post.id} delay={i * 100}>
              <Link to={`/blog/${post.id}`} className="group block bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all h-full border border-leather-100">
                <div className="aspect-[16/10] overflow-hidden">
                  <img src={processImageUrl(post.imageUrl, 600)} alt={post.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                </div>
                <div className="p-8">
                ...
                <div key={product.id} className="w-1/3 group relative">
                  <Link to={`/producto/${product.id}`} className="block">
                    <div className="aspect-[4/5] overflow-hidden rounded-lg mb-4 relative shadow-md">
                       <img src={processImageUrl(product.images[0], 600)} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
              ...
              <div key={product.id} className="group">
                  <Link to={`/producto/${product.id}`} className="block">
                    <div className="aspect-square overflow-hidden rounded-lg mb-4 relative shadow-md">
                       <img src={processImageUrl(product.images[0], 600)} alt={product.name} className="w-full h-full object-cover" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {blogPosts.map((post) => (
             <div key={post.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all h-full border border-leather-100 flex flex-col cursor-pointer" onClick={() => setSelectedPost(post.id)}>
                <div className="aspect-[16/10] overflow-hidden">
                  <img src={processImageUrl(post.imageUrl, 600)} alt={post.title} className="w-full h-full object-cover transition-transform duration-700 hover:scale-105" />
                </div>
...
const AdminPanel = () => {
  ...
  const [activeTab, setActiveTab] = useState<'products' | 'fairs' | 'history' | 'blog'>('products');
  ...
  return (
    <div className="min-h-screen bg-leather-50 pt-36 pb-12">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-lg border border-leather-200 min-h-[600px] overflow-hidden">
          <div className="border-b border-leather-200 px-8 py-5 flex gap-8 bg-leather-50/50">
            {['products', 'fairs', 'history', 'blog'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab as any)} className={`text-lg font-bold pb-1 border-b-2 capitalize ${activeTab === tab ? 'border-leather-900 text-leather-900' : 'border-transparent text-leather-400'}`}>{tab === 'products' ? 'Productos' : tab === 'fairs' ? 'Ferias' : tab === 'history' ? 'Historia' : 'Blog'}</button>
            ))}
          </div>
          <div className="p-8">
            {activeTab === 'products' && (
          ...
          {filtered.map(product => (
            <div key={product.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all group border border-leather-100">
              <Link to={`/producto/${product.id}`} className="block relative aspect-square overflow-hidden">
                <img src={processImageUrl(product.images[0], 400)} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt={product.name} />
              </Link>
              <div className="p-4">
